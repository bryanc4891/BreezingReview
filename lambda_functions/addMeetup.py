...
def lambda_handler(event, context):
    ...
    organiserId = queryParams.get('organiserid')
    attendeeId = queryParams.get('attendeeid')
    placeId = queryParams.get('placeid')
    timeOfMeeting = queryParams.get('timeofmeeting')
    queryString = f"INSERT INTO MEETUP (MeetupID, OrganiserID, AttendeeID, PlaceID, TimeOfMeeting) VALUES ('{meetupId}', '{organiserId}', '{attendeeId}', '{placeId}', '{timeOfMeeting}')"
    print(queryString)
    with conn.cursor() as cur:
        cur.execute(queryString)
    conn.commit()
    responseBody = f"'{cur.rowcount}' Meetup inserted"
    print(responseBody)
    res = {
        "statusCode": statusCode,
        "headers": headers,
