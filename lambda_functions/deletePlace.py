...
def lambda_handler(event, context):
    ...
    if queryParams is None or queryParams.get('placeid') is None:
        res["statusCode"] = 400
        res["body"] = "Parameters not provided"
        return res
        
    placeId = queryParams.get('placeid')
    queryString = f"DELETE FROM PLACE where PlaceID='{placeId}'"
    print(queryString)
    with conn.cursor() as cur:
        cur.execute(queryString)
    conn.commit()
    responseBody = f"'{cur.rowcount}' place deleted"
    print(responseBody)
    res = {
        "statusCode": statusCode,
        "headers": headers,
